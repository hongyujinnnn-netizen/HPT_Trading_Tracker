-- =====================================================================
-- MPT / TradePulse Gold — Complete Master Database Schema & Reset Script
-- PostgreSQL schema (Supabase-ready: auth.users + RLS + Multi-Account Support)
-- =====================================================================

-- ---------------------------------------------------------------------
-- 0. Clean Database Reset (Cascade Drop Existing Views & Tables)
-- ---------------------------------------------------------------------
drop view if exists public.view_mistake_cost cascade;
drop view if exists public.view_strategy_performance cascade;
drop view if exists public.view_session_performance cascade;
drop view if exists public.view_equity_curve cascade;
drop view if exists public.view_dashboard_stats cascade;
...
drop table if exists public.mistake_types cascade;
...
create table public.user_settings (
  ...
  display_name           text,
  account_balance        numeric(14,2) not null default 10000.00,
  base_currency          text not null default 'USD',
  default_risk_pct       numeric(5,2)  not null default 1.00 check (default_risk_pct > 0 and default_risk_pct <= 100),
  contract_size          numeric(10,2) not null default 100,   -- XAU/USD: $ per 1.00 price point per 1 lot
...
-- ---------------------------------------------------------------------
-- 3. trading_accounts (Sub-Accounts inside Main User Account)
-- ---------------------------------------------------------------------
create table public.trading_accounts (
  ...
  account_type    text not null default 'live', -- 'live', 'demo', 'challenge', 'backtest'
  account_number  text,
...
-- ---------------------------------------------------------------------
-- 6. trades (Core Journal Entry with Sub-Account Reference)
-- ---------------------------------------------------------------------
...
create index idx_trades_user_time   on public.trades (user_id, entry_time desc);
...
create index idx_trades_emotion     on public.trades (emotion);
...
create index idx_pending_orders_active on public.pending_orders (status) where status in ('pending', 'active');
create index idx_pending_orders_user on public.pending_orders (user_id);
create index idx_pending_orders_account_id on public.pending_orders (account_id);
...
-- ---------------------------------------------------------------------
-- 9. mistake_types & trade_mistakes
-- ---------------------------------------------------------------------
create table public.mistake_types (
  id          smallint primary key,
...
insert into public.mistake_types (id, code, label, description) values
  ...
  (4, 'late_entry',     'Late Entry',       'Entry price far from stated strategy trigger, or realized RR < 1.0.'),
...
create index idx_mistakes_trade on public.trade_mistakes (trade_id);
create index idx_mistakes_type  on public.trade_mistakes (mistake_type_id);

-- ---------------------------------------------------------------------
-- 10. economic_events & price_snapshots & csv_imports
-- ---------------------------------------------------------------------
...
create index idx_price_snapshots_time on public.price_snapshots (symbol, captured_at desc);
...
-- Auto-calculate PnL and Risk Amount on trade insert/update
...
-- Automatic Mistake Detector Engine
...
-- Auto-Provisioning User Settings, Strategies, and Default Trading Sub-Account on Signup
...
begin
  ...
  -- 3. Create Default Primary Trading Sub-Account
  insert into public.trading_accounts (user_id, name, broker, account_type, initial_balance, leverage, color_hex, is_default)
...
create policy "own trading_accounts" on public.trading_accounts for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
-- Public Read Reference Policies
...
-- =====================================================================
-- 13. Dashboard & Analytical PostgreSQL Views
-- =====================================================================
